...
export function useChat() {
  ...
  const [deepseekSaveResult, setDeepseekSaveResult] = useState<'idle' | 'saved' | 'error'>('idle');
  ...
  useEffect(() => {
    ...
    onBackendMessage('api_key_status', (payload) => {
      const data = payload as {
        ...
        deepseek?:  { configured?: boolean; maskedKey?: string };
      ...
      setDeepseekConfigured(data?.deepseek?.configured ?? false);
      setDeepseekMasked(data?.deepseek?.maskedKey ?? '');
    ...
    onBackendMessage('api_key_save_result', (payload) => {
      ...
      switch (data?.provider) {
        ...
        case 'deepseek':  setDeepseekSaveResult(status);  setTimeout(() => setDeepseekSaveResult('idle'), 3000);  break;
  ...
  const saveDeepSeekApiKey = useCallback((apiKey: string) => {
    sendToBackend('save_deepseek_api_key', { apiKey });
  }, []);
